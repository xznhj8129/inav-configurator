...
class MavlinkMspTunnel {
    ...
    wrapMspFrame(frameBuffer, code) {
        ...
        console.log('mavlinkTunnel txMsp code=' + code + ' mspBytes=' + frameBytes.length + ' tunnelFrames=' + packets.length + ' targetSysid=' + this._targetSystemId);
            ...
            if (msgId === MAVLINK_MSG_ID_TUNNEL) {
                ...
                if (this._isConfiguratorTunnel(payload, systemId, componentId)) {
                    ...
                    if (payload.length < expectedPayloadLength) {
                        console.log('mavlinkTunnel rxChunkPadded sysid=' + systemId + ' compid=' + componentId + ' expected=' + expectedPayloadLength + ' actual=' + payload.length + ' padded=' + (expectedPayloadLength - payload.length));
                    }
                    console.log('mavlinkTunnel rxChunk sysid=' + systemId + ' compid=' + componentId + ' target=' + payload[2] + ':' + payload[3] + ' mspBytes=' + tunnelLength);
    ...
    _acceptHeartbeat(systemId, payload) {
        ...
        if (!this._targetSystemId) {
            ...
            console.log('mavlinkTunnel heartbeat sysid=' + systemId + ' requestedSysid=' + this._requestedSystemId + ' autopilot=' + payload[5]);
            return true;
        }
